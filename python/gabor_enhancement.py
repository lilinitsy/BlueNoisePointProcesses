...
def precompute_impulses_bluenoise(tex_w: int, tex_h: int,
                                   num_cells: int, num_impulses: int,
                                   rng_seed: int,
                                   bn_points_uv: np.ndarray,
                                   bias_strength: float = 0.5):
	cell_h = tex_h // num_cells
	cell_w = tex_w // num_cells
 
	bn_px = bn_points_uv[:, 0] * tex_w   # (N,)
	bn_py = bn_points_uv[:, 1] * tex_h   # (N,)
	...
	for cy in range(num_cells):
		for cx in range(num_cells):
			...
			for i in range(num_impulses):
				...
				# Find nearest blue noise point and lerp
				dx2 = (bn_px - ix) ** 2
				dy2 = (bn_py - iy) ** 2
				nearest = int(np.argmin(dx2 + dy2))
				ix = ix * (1.0 - bias_strength) + bn_px[nearest] * bias_strength
				iy = iy * (1.0 - bias_strength) + bn_py[nearest] * bias_strength
				ix = float(np.clip(ix, x0, x1 - 1))
				iy = float(np.clip(iy, y0, y1 - 1))
...
def gabor_foveated_enhance_bluenoise(
	image_path: str,
	blue_noise_texture: np.ndarray,
	foveation_center: tuple[float, float] = (0.5, 0.5),
	radius_fovea: float                   = 0.1,
	radius_periphery: float               = 0.2,
	screen_width_cm: float                = 60.0,
	...
	uv_grid        = np.stack([uv_x, uv_y], axis=-1)
	phys_diff      = (uv_grid - fov_uv) * screen_dims_cm
	ecc_deg_map    = np.linalg.norm(np.degrees(np.arctan(phys_diff / distance_from_screen_cm)), axis=-1).astype(np.float32)
 
	dist_px   = np.sqrt((px_x_np - gaze_px[0])**2 + (px_y_np - gaze_px[1])**2)
	sigma_map = np.maximum(EPSILON, (blur_rate_arcmin_per_degree * (dist_px / ppd) / 60.0 * ppd) / 2.355).astype(np.float32)
	...
	mu_n    = 0.5 * (np.log(np.maximum(EPSILON, F_L_map)) + np.log(np.maximum(EPSILON, F_H_map)))
	sigma_n = (0.5 * s_f * (mu_n - np.log(np.maximum(EPSILON, F_L_map)))).astype(np.float32)
	...
	print(f"Precomputing blue-noise-biased impulses ({cells}×{cells} cells × {impulses_per_cell}) …")
	imp_x_np, imp_y_np, imp_rn_np = precompute_impulses_bluenoise(W, H, cells, impulses_per_cell, seed, bn_points_uv=np.asarray(blue_noise_texture, dtype=np.float32),)
...
def precompute_impulses_bluenoise_tiled(tex_w: int, tex_h: int,
                                         num_cells: int,
                                         rng_seed: int,
                                         bn_points_uv: np.ndarray):
	"""
	Each cell gets exactly one impulse per blue noise point, with the point
	scaled from [0,1]² into that cell's pixel bounds.
	bn_points_uv : (N, 2) — defines both the positions and impulses_per_cell
	"""
	num_impulses = bn_points_uv.shape[0]
	cell_h = tex_h // num_cells
	cell_w = tex_w // num_cells
	...
	for cy in range(num_cells):
		for cx in range(num_cells):
			...
			for i in range(num_impulses):
				...
				ix = x0 + bn_points_uv[i, 0] * (x1 - x0)
				iy = y0 + bn_points_uv[i, 1] * (y1 - y0)
...
def gabor_foveated_enhance_bluenoise_tiled(
	image_path: str,
	blue_noise_texture: np.ndarray,
	foveation_center: tuple[float, float] = (0.5, 0.5),
	radius_fovea: float                   = 0.1,
	...
	img  = np.array(Image.open(image_path).convert("RGB"), dtype=np.float32) / 255.0
	H, W = img.shape[:2]
	bn   = np.asarray(blue_noise_texture, dtype=np.float32)
	impulses_per_cell = bn.shape[0]
	print(f"Image  : {W}×{H}    Impulses/cell : {impulses_per_cell}")
	...
	print(f"Precomputing tiled blue-noise impulses ({cells}×{cells} cells × {impulses_per_cell}) …")
	imp_x_np, imp_y_np, imp_rn_np = precompute_impulses_bluenoise_tiled(
		W, H, cells, seed, bn
	)
